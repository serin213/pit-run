...
const TOP_GAP = 32; // was 12 — 20px lower per spec
...
function CheckCertSvg() {
  return (
    <Svg width={20} height={21} viewBox="0 0 24 24" fill="none">
      <Path d={CHECK_CERT_PATH} fill={PALETTE.red} />
    </Svg>
  );
}
...
export default function FeedbackToast({ visible, message, onDismiss, icon }: Props) {
  ...
  useEffect(() => {
    ...
    // Soft spring bounce on slide-in
  ...
  return (
    <Animated.View
      ...
      style={[
        styles.wrap,
        { top: safeTop + TOP_GAP, opacity: anim, transform: [{ translateY }] },
      ]}
    >
